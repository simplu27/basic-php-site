 ...
 * 2. If POST: handle user input.
 *      On Success: send email, redirect to thank you message by sending a GET with ?status=thanks
 *      On Failure: prompt error message, and show the data user typed before in the form again.
 * 3. If Not POST: Do we have GET?
 * 4.       YES: check if status=thanks. If so, display thank you message.
 *          NO:  display the form (original blank form).
 */
		...
		<?php } else { ?>
			...
			<form method="post" action="suggest.php">
				<table>
					...
					<!-- Display different drop downs groups. -->
					<tr>
						<th>
					...
					<tr>
						...
						<td><textarea id="details" name="details"><?php if (isset($details)) {
									echo $details;
								} ?></textarea></td>
					</tr>
